...
(function (root, factory) {
  ...
  function getActiveBranchMessages(payload) {
    ...
    if (!leafUuid || !byUuid.has(leafUuid)) {
      ...
      if (!root) {
        // Flat payload: no message carries tree linkage at all, so there is no
        // branch to walk. Fall back to array order rather than dropping every
        // message. A payload that *has* linkage but no resolvable root still
        // returns [] so buildExportDiagnostics can report it.
        const hasTreeLinkage = messages.some((m) => m.parent_message_uuid);
        return hasTreeLinkage ? [] : messages.slice();
      }
      const branch = [];
      let node = root;
      while (node) {
  ...
  function findAssistantTurns(doc) {
    ...
    for (const button of statusButtons) {
      const host =
        ...
        button.closest('[data-testid="transcript-row"]') ||
  ...
  function buildExportDiagnostics(payload, options = {}) {
    ...
    if (!normalized) {
      ...
      const total = normalized.chat_messages.length;
      diagnostics.skipped.push({
        category: "transcript",
        reason: total
          ? `Payload has ${total} message(s) but the active branch resolved to 0 (tree linkage missing or broken?)`
